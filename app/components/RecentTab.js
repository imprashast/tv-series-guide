...
export default class RecentTab extends Component {
    ...
    componentWillMount() {
        ...
        let query = 'SELECT e.firstAired,e.season,e.number,e.tvdb,e.imageURL,e.title,s.title AS seriesTitle, s.tvdb AS parentTvdb, e.seen  FROM episode e LEFT OUTER JOIN shows s ON s.trakt = e.showTraktID WHERE e.firstAired <= ? ORDER BY e.firstAired DESC ';
